Writeups de Estudo, Segurança Defensiva e SOC

Registro dos meus estudos práticos em segurança, seguindo trilhas do TryHackMe e
laboratórios próprios. Cada writeup é escrito com minhas próprias palavras, focando
no raciocínio de análise, não em colar a resposta.

👤 Autor: Pedro Alves · [LinkedIn](https://www.linkedin.com/in/pedroalvesc/)
🎯 Objetivo: Analista de SOC evoluindo para Cyber Security Engineer

---

## ⭐ Destaques, comece por aqui

| Writeup | O que demonstra | MITRE ATT&CK |
|---|---|---|
| [Snort Challenge, criação e debug de regras IDS](writeups/snort-challenge-basics) | Escrita de regras Snort, detecção de exploits reais, troubleshooting | T1210 EternalBlue MS17-010, T1190 Log4Shell |
| Neste laboratório escrevi regras de IDS com Snort do zero, cobrindo detecção de tráfego HTTP e FTP, identificação de arquivos por magic bytes e correção de erros de sintaxe. Depois apliquei regras contra capturas de exploits como EternalBlue e Log4Shell, cruzando os alertas com tcpdump e base64 pra confirmar manualmente o que cada regra encontrou. O foco foi entender a anatomia de uma regra e a diferença entre detectar um protocolo e detectar um evento específico dentro dele.| | |

## 🛡️ Defensive Security e SOC
- [SOC L1, Alert Triage](23-alert-triage.md)
- [SOC L1, Alert Reporting](24-alert-reporting.md)
- [Logs Fundamentals](07-logs-fundamentals.md)
- [Introdução a EDR](17-intro-edr.md)
- [Introdução a SOAR](18-intro-soar.md)
- [SOC Metrics](26-soc-metrics.md) · [Workbooks e Lookups](25-workbooks-lookups.md)

## 🔎 SIEM e Detecção
- [Introdução a SIEM](08-siem.md)
- [Splunk Basics](21-splunk-basics.md)
- [Elastic Stack Basics](22-elastic-stack-basics.md)
- [IDS e IPS Fundamentals](10-ids-ips-fundamentals.md)
- [Firewall](09-firewall.md)

## 🧠 Frameworks
- [Pyramid of Pain](19-pyramid-of-pain.md)
- [Cyber Kill Chain](20-cyber-kill-chain.md)

## 🌐 Redes e Análise de Tráfego
- [Networking, Concepts e Essentials](networking-concepts-essentials.md)
- [Networking, Core e Secure Protocols](networking-core-secure-protocols.md)
- [Wireshark Basics](wireshark-basics.md) · [tcpdump Basics](tcpdump-basics.md) · [Nmap Basics](nmap-basics.md)

## 🐧 Sistemas e Ferramentas
- [Linux Fundamentals](11-linux-fundamentals.md) · [Linux Shells](12-linux-shells.md)
- [Criptografia e Public Key](03-cryptography-e-public-key.md) · [Hashing Basics](07-hashing-basics-github.md)
- [CyberChef](cyberchef-basics.md) · [CAPA](capa-basics.md) · [REMnux](remnux-getting-started.md) · [FLARE VM](13-flarevm-arsenal-of-tools.md)

## 🔓 Phishing e Vulnerabilidades
- [Introdução a Phishing](27-intro-phishing.md)
- [Vulnerability Management](11-vulnerability-management.md)

---
_Em construção: writeups do meu home lab de detecção com Sysmon, SIEM e simulação de ataques._
